...
async def _run_realtime(source_name: str, ui_mode: str = "cli") -> None:
    ...
    if source_name == "recall":
        from transcription.recall_source import make_recall_source
        from coverage.tracker import CoverageTracker
        ...
        try:
            ...
            webhook_host  = getattr(cfg, "webhook_host",          webhook_host)
            webhook_port  = int(getattr(cfg, "webhook_port",      webhook_port))
            recall_secret = getattr(cfg, "recall_webhook_secret", recall_secret)
        except (ValueError, Exception):
            pass

        source = make_recall_source(host=webhook_host, port=webhook_port, secret=recall_secret)

        print("\n" + "=" * 60)
        print("  🚀 DIAGNÓSTICO EM TEMPO REAL — CITi")
        print("=" * 60)
        print(f"Fonte: Recall.ai bot")
        print(f"Webhook: http://{webhook_host}:{webhook_port}/transcription")
        if ui_mode == "web":
            print("UI: browser — http://127.0.0.1:8080")
        else:
        ...
        await source.server.start()
        print(f"\n✅ Aguardando webhook do Recall.ai em http://{webhook_host}:{webhook_port}/transcription")
        print("   Configure essa URL no dashboard do Recall.ai ao criar o bot.")
        if recall_secret:
            print(f"   Lembre de enviar o header: Authorization: Bearer {recall_secret[:6]}...")
        print()
...
def main() -> None:
    ...
    parser.add_argument(
        "--source",
        choices=["taqtic", "file", "stdin", "recall"],
        default="taqtic",
