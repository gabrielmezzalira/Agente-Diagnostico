...
async def create_project(payload: ProjectCreate, db: Client = Depends(get_supabase)):
    ...
    row = payload.model_dump(mode="json", exclude={"gemini_api_key", "pricing_api_key"})
    row["gemini_api_key_secret_id"] = secret_id
    if payload.pricing_api_key:
        row["pricing_api_key_secret_id"] = _vault_store(
            db, payload.pricing_api_key, f"pricing-{payload.client}"
        )
...
async def update_project(
    ...
    existing = (
        db.table("projects")
        .select("id, gemini_api_key_secret_id, pricing_api_key_secret_id")
        .eq("id", str(project_id))
        .execute()
    )
    ...
    updates = payload.model_dump(
        mode="json", exclude_none=True, exclude={"gemini_api_key", "pricing_api_key"}
    )
    ...
    if payload.pricing_api_key:
        old_pricing_id = existing.data[0].get("pricing_api_key_secret_id")
        if old_pricing_id:
            _vault_delete(db, old_pricing_id)
        updates["pricing_api_key_secret_id"] = _vault_store(
            db, payload.pricing_api_key, f"pricing-{project_id}"
        )
...
async def delete_project(project_id: UUID, db: Client = Depends(get_supabase)):
    ...
    pricing_secret_id = existing.data[0].get("pricing_api_key_secret_id")
    if pricing_secret_id:
        _vault_delete(db, pricing_secret_id)
