...
class PromptBuilder:
    ...
    def build_report_generator(self) -> str:
        ...
        return (
            ...
            "IMPORTANTE: siga EXATAMENTE esta ordem de seções. As primeiras seções (Cobertura, Arquitetura, "
            "Sprints, Recomendação) são as mais críticas para precificação e escopo — NÃO as truncue. "
            "Seja conciso em cada seção; o relatório completo não deve ultrapassar 2000 palavras.\n\n"
            "Estruture o relatório com estas seções na ordem exata:\n"
            "## Cobertura por Área\n"
            "  Tabela: Área | Status | Score | Observações\n"
            "  OBRIGATÓRIO: inclua TODAS as áreas aplicáveis na tabela, mesmo com score 0 e status uncovered.\n"
            "  Omita apenas áreas marcadas como not_applicable.\n"
            "## Nível de Complexidade\n"
            "  - Classificação: Simples | Médio | Complexo | Bomba\n"
            "  - Justificativa em no máximo 3 bullets\n"
            ...
            "    Omita camadas não aplicáveis.\n"
            "  ### Componentes e Fluxo de Dados\n"
            "    - Módulos principais numerados (máximo 6)\n"
            "    - Fluxo em uma linha: Módulo A → Módulo B → ...\n"
            "  ### Possíveis Armadilhas\n"
            "    - Máximo 3 riscos arquiteturais específicos deste projeto\n"
            "## Estrutura de Sprints Recomendada\n"
            "  Selecione apenas módulos do catálogo CITi relevantes.\n"
            ...
            "  - Próximos passos concretos numerados (máximo 5)\n"
            "## Riscos e Alertas\n"
            ...
            "## Maturidade de Dados\n"
            "  - Nível atual, gap e impacto no projeto (máximo 3 bullets)\n"
            "## Perguntas Ainda Sem Resposta\n"
            "  - Liste o que ficou sem resposta e precisa ser esclarecido antes de fechar\n"
            + (
            "## Diagnóstico Pré-Reunião vs Realidade\n"
            "  (última seção — seja conciso, máximo 3 bullets por sub-item)\n"
            "  Use o bloco '## Contexto pré-reunião estruturado' da mensagem do usuário.\n"
            "  - **O que já se sabia:** dores, stack, restrições levadas para a reunião\n"
            "  - **O que a reunião confirmou:** validações da transcrição\n"
            "  - **O que mudou ou surpreendeu:** divergências entre esperado e revelado\n"
            "  - **Riscos não discutidos com o cliente:** riscos NÃO discutidos — inclua nos Próximos Passos\n"
            if self.sc else ""
            )
        )
